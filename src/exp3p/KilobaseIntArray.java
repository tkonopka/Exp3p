...
/**
 * A class that is meant to work like an integer array, but can save memory space if
 * the values stored are sparse. For example, it is useful to store a coverage track 
 * that has zeros in many long stretches. 
 *
 * Implementation - a large array is split up into blocks/chunks/buckets of 1 kilobase each.
 * Memory is allocated for each kilobase only when it is needed. Lookup and
 * assignment at each location is requires splitting a canonical array index into 
 * a chunk and an offset index, so these operations should be expected to be slightly slower
 * than in a primitive array.
 *
 *
 * @author tkonopka
 */
public class KilobaseIntArray {
     ...
     * @return 
     * 
     * the number of chunks that have been assigned in memory.
     * This is a proxy for the memory usage of the object.
     * 
     * 
     */
     ...
     * @return 
     * 
     * the value originally used in the constructor, i.e. the number of elements that
     * can be stored in the array (actual number of elements may actually be larger because chunks
     * are always allocated by kb)
     */
    public int size() {
        return totlength;
    }
